...
import { ERROR, LOADING, SETUP_USER_ERROR, SUCCESS, SUCCESS_NO_MESSAGE } from "../actions";
...
export const chagePassword = ({ currentPassword }) => async (dispatch) => {
  dispatch({ type: LOADING });
  try {

    const { data } = await authFetch.put('/account/password', currentPassword);
